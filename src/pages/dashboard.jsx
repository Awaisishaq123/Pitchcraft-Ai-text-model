...
//   // 🔹 Fetch pitches safely
//   useEffect(() => {
//     if (!userId) return; // ✅ Prevent early Firestore call

//     const fetchPitches = async () => {
//       try {
//         setLoading(true);
//         console.log("Fetching pitches for userId:", userId);
...
//               <p>
//                 <strong>Created:</strong>{" "}
//                 {selectedPitch.createdAt?.seconds
//                   ? new Date(selectedPitch.createdAt.seconds * 1000).toLocaleString()
//                   : "Unknown"}
//               </p>
//             </div>
...
export default function Dashboard() {
  // ✅ Load darkMode from localStorage on start
  ...
  // ✅ Save darkMode to localStorage whenever changed
  useEffect(() => {
    localStorage.setItem("theme", darkMode ? "dark" : "light");
  }, [darkMode]);

  // 🔹 Listen for auth changes (ensure userId not undefined)
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setUserName(user.displayName || "User");
  ...
  // 🔹 Fetch pitches (only after userId is set)
  useEffect(() => {
    if (!userId) return;
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                  {pitches.map((pitch) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => openModal(pitch)}
                        className="px-4 py-2 rounded-lg bg-linear-to-r from-indigo-500 to-pink-500 text-white hover:from-indigo-600 hover:to-pink-600 transition-all shadow-md hover:shadow-lg"
                      >
                        View
                      </button>
                    </div>
